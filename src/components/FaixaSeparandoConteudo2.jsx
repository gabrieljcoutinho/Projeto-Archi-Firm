import React from 'react'
import "../../src/Css/faixaLaranjaSeparandoConteudo2.css"


const FaixaSeparandoConteudo2 = () => {
  return (
    <div className='FaixaSeparandoConteudo2'>

      <h2 className='tituloSeparandoConteudo'>Start your architectural journey < br/>
        with us</h2>

        <ul className='listaDosbtns'>

        <li className='listaLinksDosBtns'><button className='btnLeranMore'>Learn more</button></li>
        <li className='listaLinksDosBtns'><button className='btnContact'>Contact Us</button></li>

        </ul>

      </div>
  )
}

export default FaixaSeparandoConteudo2